...
function create(req, res) {
    ...
    fotografias.create(body)
        ...
        .catch(err => {
            res.status(500).send({ message: 'Ocurrio error al guardar la fotografia', err });
        });
}
function update(req, res) {
    var id_fotografia = req.params.id;
    var body = req.body;
    fotografias.findByPk(id_fotografia)
        .then(fotografia => {
...
function uploadFotografia(req, res) {
    ...
    if (req.files) {
        var file_path = req.files.foto.path;        // foto: es con lo que subo
                    ...
                    fotografia.update(foto)
                        .then( () => {
                            ...
                            thumb({
                                ...
                                res.status(500).send({ message: 'Ocurrio un error al crear el thumbnail', err });
                            });
                            res.status(200).send({ fotografia });
                        })
                        .catch(err => {
