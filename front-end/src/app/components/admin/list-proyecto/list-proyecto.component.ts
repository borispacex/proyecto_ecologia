  ...
  agregar() {
    ...
    .then(responseProy => {
      ...
      if (this.filesToUpload) {
        this.filesToUpload.forEach(archivoSubir => {
          var proyecto_archivo: any = {
            ...
            id_tipo: 1,
            nombre: this.obtenerNombreArchivo(archivoSubir.name)
          };
          this._serviceProyArch.saveProy_archivo(proyecto_archivo, this.token)
          .then(responseProyArch => {
            this._uploadArchivo.uploadArchivo(this.url + 'upload-proyecto-archivo/' + responseProyArch.proy_archivo.id_proy_archivo, archivoSubir, this.token)
            .then((responseArchivo: any) => {
              console.log(responseArchivo);
              if (sw) { this.toastr.success('Proyecto guardado', undefined, { closeButton: true, positionClass: 'toast-bottom-right' }); sw = false; }
            }).catch(error => {
